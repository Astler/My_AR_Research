...
namespace Niantic.ARVoyage.Walkabout
{
    ...
    /// a locomotion destination target for Doty to roll their snowball to.
    ...
    /// </summary>
    public class StateBuildSnowman : StateBase
    {
        public const float closeToSnowmanDist = 0.5f;
        ...
        void Update()
        {
            ...
            if (!buildingSnowman &&
                    walkaboutManager.IsYetiSnowballNearTarget(
                            walkaboutManager.snowman.transform.position, nearDist: closeToSnowmanDist))
            {
