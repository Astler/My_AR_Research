...
namespace Niantic.ARVoyage
{
    ...
    {
        [Tooltip(
            "These clips will loop throughout the scene. They will automatically fade out when leaving the scene.")]
        [SerializeField]
        private List<ClipData> persistentAmbientClips;

        [Tooltip("The clips available to play in this scene.")] [SerializeField]
        private List<AudioClipList> audioClipLists;

        [Tooltip("Should the list of audio key constants be copied to the clipboard on run?")] [SerializeField]
        private bool buildKeyConstsToClipboardInEditor;

        private readonly Dictionary<string, AudioVariants> AudioDictionary = new();
        ...
        private void Awake()
        {
            ...
            Fader.FadingSceneOut += OnFadingSceneOut;
            Fader.FadingSceneIn += OnFadingSceneIn;
        }
        ...
        private void OnFadingSceneIn(float fadeDuration)
        {
            for (int i = 0; i < persistentAmbientClips.Count; i++)
            {
                ...
                PlayAudioOnPoolSource(clipData.clip, targetTransform: transform, position: Vector3.zero,
                    volume: clipData.volume, spatialBlend: 0f, loop: true, fadeInDuration: fadeDuration, dspDelay: 0);
            }
        }
        ...
        public Coroutine FadeInAudioSource(AudioSource source, float fadeDuration, float targetVolume,
            Func<float, float> easingFunc = null)
        {
            return FadeAudioSource(source, fadeDuration, targetVolume, easingFunc);
        }

        public Coroutine FadeOutAudioSource(AudioSource source, float fadeDuration,
            Func<float, float> easingFunc = null)
        {
            return FadeAudioSource(source, fadeDuration, targetVolume: 0f, easingFunc);
        }

        private Coroutine FadeAudioSource(AudioSource source, float fadeDuration, float targetVolume,
            Func<float, float> easingFunc)
        {
            ...
            return InterpolationUtil.EasedInterpolation(
                ...
                onUpdate: progress => { source.volume = Mathf.Lerp(startVolume, targetVolume, progress); },
                onComplete: () =>
                {
        ...
        public AudioSource PlayAudioNonSpatial(string audioKey, float volume = 1, bool loop = false,
            float fadeInDuration = 0, double dspDelay = 0)
        {
            return PlayAudioOnPoolSource(audioKey, targetTransform: transform, position: Vector3.zero, volume,
                spatialBlend: 0f, loop, fadeInDuration, dspDelay);
        }
        ...
        /// <param name="dspDelay">How long to delay starting the clip, in DSP time. N.B. fade in doesn't wait for this delay.</param>
        /// <returns>The source playing the audio.</returns>
        public AudioSource PlayAudioAtPosition(string audioKey, Vector3 position, float volume = 1,
            float spatialBlend = 1, bool loop = false, float fadeInDuration = 0, double dspDelay = 0)
        {
            return PlayAudioOnPoolSource(audioKey, targetTransform: null, position, volume, spatialBlend, loop,
                fadeInDuration, dspDelay);
        }
        ...
        /// <param name="dspDelay">How long to delay starting the clip, in DSP time. N.B. fade in doesn't wait for this delay.</param>
        /// <returns>The source playing the audio.</returns>
        public AudioSource PlayAudioOnObject(string audioKey, GameObject targetObject, float volume = 1,
            float spatialBlend = 1, bool loop = false, float fadeInDuration = 0, double dspDelay = 0)
        {
            return PlayAudioOnPoolSource(audioKey, targetTransform: targetObject.transform, Vector3.zero, volume,
                spatialBlend, loop, fadeInDuration, dspDelay);
        }
        ...
        public SegmentedAudioLoop PlaySegmentedLoop(string audioKeyIn, string audioKeyLoop, string audioKeyOut,
            float volume = 1, float stopCrossfadeDuration = .5f)
        {
            return SegmentedAudioLoop.CreateAndStart(this, audioKeyIn, audioKeyLoop, audioKeyOut, volume,
                stopCrossfadeDuration);
        }

        // Wrapper that takes an audioKey
        private AudioSource PlayAudioOnPoolSource(string audioKey, Transform targetTransform, Vector3 position,
            float volume, float spatialBlend, bool loop, float fadeInDuration, double dspDelay)
        {
            ...
            return PlayAudioOnPoolSource(clip, targetTransform, position, volume, spatialBlend, loop, fadeInDuration,
                dspDelay);
        }

        public AudioSource PlayAudioOnPoolSource(AudioClip clip, Transform targetTransform, Vector3 position,
            float volume, float spatialBlend, bool loop, float fadeInDuration, double dspDelay)
        {
            Debug.LogFormat(
                "{0} PlayAudioOnPoolSource: [clip {1}] [target {2}] [volume {3}] [spatialBlend {4}] [loop {5}]",
                this,
                clip.name,
        ...
        {
            GameObject audioSourceGO = new("AudioManagerPool_" + audioSourcePool.Count);
