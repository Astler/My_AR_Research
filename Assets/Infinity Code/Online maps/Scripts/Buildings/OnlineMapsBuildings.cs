...
public class OnlineMapsBuildings : MonoBehaviour, IOnlineMapsSavableComponent
{
    ...
    [NonSerialized] public Dictionary<string, OnlineMapsBuildingBase> buildings;

    [NonSerialized] public GameObject container;

    [NonSerialized] public OnlineMapsControlBaseDynamicMesh control;
    ...
    [NonSerialized] public OnlineMaps map;
    ...
    public void CreateBuilding(OnlineMapsBuildingsNodeData data)
    {
        ...
        if (building != null)
        {
            ...
            building.gameObject.layer = map.gameObject.layer;
        }
        else
        {
    ...
    private void GenerateBuildings()
    {
        ...
        lock (newBuildingsData)
        {
            ...
            if (needCreate > 0 &&
                (newBuildingIndex == 0 ||
                 (maxBuilding > 0 && unusedBuildings.Count + buildings.Count >= maxBuilding)) &&
    ...
    private void MoveRelationToWay(OnlineMapsOSMRelation relation, Dictionary<string, OnlineMapsOSMWay> ways,
        List<string> waysInRelation, Dictionary<string, OnlineMapsOSMNode> nodes)
    {
        ...
        List<OnlineMapsOSMRelationMember> members =
            relation.members.Where(m => m.type == "way" && m.role == "outer").ToList();
    ...
    private static bool MoveRelationMemberToWay(List<string> nodeRefs, List<OnlineMapsOSMRelationMember> members,
        Dictionary<string, OnlineMapsOSMWay> ways)
    {
    ...
    public void MoveRelationsToWays(List<OnlineMapsOSMRelation> relations, Dictionary<string, OnlineMapsOSMWay> ways,
        Dictionary<string, OnlineMapsOSMNode> nodes)
    {
    ...
    private void RequestNewBuildings()
    {
        ...
        requestData = string.Format(OnlineMapsUtils.numberFormat,
            "(way[{4}]({0},{1},{2},{3});relation[{4}]({0},{1},{2},{3}););out;>;out skel qt;",
            bry, tlx, tly, brx, "'building'");
        if (OnPrepareRequest != null)
            requestData = OnPrepareRequest(requestData, new Vector2((float)tlx, (float)tly),
                new Vector2((float)brx, (float)bry));
    }
    ...
    private void UpdateBuildingsPosition()
    {
        ...
        float maxDistance = (Mathf.Pow((map.width / OnlineMapsUtils.tileSize) >> 1, 2) +
                             Mathf.Pow((map.height / OnlineMapsUtils.tileSize) >> 1, 2)) * 4;
        ...
        foreach (KeyValuePair<string, OnlineMapsBuildingBase> building in unusedBuildings)
        {
            ...
            {
                double bx, by;
                map.projection.CoordinatesToTile(value.centerCoordinates.x, value.centerCoordinates.y, map.zoom, out bx,
                    out @by);
    ...
    private static void UpdateBuildingPosition(OnlineMapsBuildingBase building,
        OnlineMapsControlBaseDynamicMesh control, double tlx, double tly, double brx, double bry)
    {
        Vector3 newPosition = control.GetWorldPositionWithElevation(building.centerCoordinates.x,
            building.centerCoordinates.y, tlx, tly, brx, bry);
    ...
    private void UpdateBuildingsScale()
    {
        ...
        UpdateBuildingsPosition();
        foreach (KeyValuePair<string, OnlineMapsBuildingBase> building in buildings)
            UpdateBuildingScale(building.Value);
        foreach (KeyValuePair<string, OnlineMapsBuildingBase> building in unusedBuildings)
            UpdateBuildingScale(building.Value);
    }
